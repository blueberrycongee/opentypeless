...
const bridge: OpenTypelessBridge = {
  ...
  getDesktopStatus: () =>
    ipcRenderer.invoke(ipcChannels.getDesktopStatus) as Promise<DesktopStatus>,
  ...
  openPermissionSettings: (kind: DesktopPermissionKind) =>
    ipcRenderer.invoke(ipcChannels.openPermissionSettings, kind) as Promise<void>,
  listDictationSessions: () =>
    ipcRenderer.invoke(ipcChannels.listDictationSessions) as Promise<DictationSession[]>,
  getDictationSession: (sessionId: string) =>
    ipcRenderer.invoke(
      ipcChannels.getDictationSession,
      sessionId,
    ) as Promise<DictationSession | null>,
  ...
  completeDictationSession: (sessionId: string) =>
    ipcRenderer.invoke(
      ipcChannels.completeDictationSession,
      sessionId,
    ) as Promise<CompleteDictationResult>,
  listSentMessages: () =>
    ipcRenderer.invoke(ipcChannels.listSentMessages) as Promise<SentMessage[]>,
  onRecordingCommand: (callback: (command: RecordingCommand) => void) => {
    const listener = (_event: Electron.IpcRendererEvent, command: RecordingCommand) =>
      callback(command);
  ...
  onDesktopAttention: (callback: (event: DesktopAttentionEvent) => void) => {
    const listener = (_event: Electron.IpcRendererEvent, event: DesktopAttentionEvent) =>
      callback(event);
